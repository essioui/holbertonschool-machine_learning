...
class Decision_Tree():
    ...
    def Gini_split_criterion_one_feature(self, node, feature):
        ...
        target_labels = self.target[node.sub_population]

        Left_F = Left_F[:, :, None] & (target_labels[:, None] == class_labels[None, :, None])
        ...
        right_gini = 1 - np.sum((right_counts / right_counts.sum(axis=1, keedims=True)) ** 2, axis=1)

        weights = left_counts.sum(axis=1) / len(node.sub_population)
        gini_average = weights * left_gini + (1 - weights) * right_gini
