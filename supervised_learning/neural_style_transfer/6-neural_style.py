...
class NST:
    ...
    def style_cost(self, style_outputs):
        ...
        if (not isinstance(style_outputs, list) or
            ...
            raise TypeError(
                f"style_outputs must be a list with a length "
                f"of {len(self.style_layers)}"
            )

        style_costs = []
        weight = 1.0 / len(self.style_layers)

        for gram_target, style_output in zip(self.gram_style_features,
                                             style_outputs):
            gram_generated = self.gram_matrix(style_output)
            cost = tf.reduce_mean(tf.square(gram_generated - gram_target))
            style_costs.append(weight * cost)
    ...
    def content_cost(self, content_output):
        ...
        if (not isinstance(content_output, (tf.Tensor, tf.Variable))
            ...
            raise TypeError(
                f"content_output must be a tensor of "
                f"shape {self.content_feature.shape}"
            )

        cost = tf.reduce_mean(tf.square(self.content_feature - content_output))
        return cost
